...
namespace UltraEd
{
	...
	void Scene::Render(LPDIRECT3DDEVICE9 target, LPDIRECT3DTEXTURE9* texture)
	{
		...
		View* view = GetActiveView();
		D3DXMATRIX matrix = view->GetViewMatrix();
	...
	void Scene::ScreenRaycast(ImVec2 screenPoint, D3DXVECTOR3* origin, D3DXVECTOR3* dir)
	{
		...
		D3DXMATRIX matrix = view->GetViewMatrix();
		D3DXVec3Unproject(&v1, &start, &viewport, &matProj, &matrix, &matWorld);

		D3DXVECTOR3 v2;
		D3DXVECTOR3 end = D3DXVECTOR3(screenPoint.x, screenPoint.y, 1.0f);
		D3DXVec3Unproject(&v2, &end, &viewport, &matProj, &matrix, &matWorld);

		*origin = v1;
		D3DXVECTOR3 v3 = v2 - v1;
		D3DXVec3Normalize(dir, &v3);
	}
